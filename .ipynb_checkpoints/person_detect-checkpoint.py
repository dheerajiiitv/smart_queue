...
#         queue.add_queue([1000, 1, 1264, 461])
        ...
        out = cv2.VideoWriter(os.path.join(output_path, 'output_video.mp4'),cv2.VideoWriter_fourcc(*'mp4v'), 24, (width, height))
        frame_count = 0
        while cap.isOpened() :
            ret, frame=cap.read()
...
#                 cv2.imshow("frame",s image)
        ...
        with open(os.path.join(output_path,"stats.txt"),'w') as f:
            f.write("Running model on Device  - "+device + "\n" + output_data + "\n" + "Average Inference time - " + str(avg_inference_time/frame_count) + "\nAverage loading time - " + str(model_loading_time))
        out.release()
        cap.release()
